...
class TestAdapter:
    ...
    def test_registry_get_timedelta(self):
        ...
        def add_route_and_views(config):
            config.add_route("dbname", "/test/")
            config.add_view(get_timedelta, route_name="dbname", renderer="json")
            json_renderer = JSON()
        ...
        assert (
            round(
                timedelta_adapter_factory(TimeDeltaModes.HOURS)(delta, None), 8
            )
            == 255.06259855
        )
        assert (
            round(
                timedelta_adapter_factory(TimeDeltaModes.DAYS)(delta, None), 8
            )
            == 10.62760827
        )
        assert (
            round(
                timedelta_adapter_factory(TimeDeltaModes.WEEKS)(delta, None), 8
            )
            == 1.51822975
        )
