...
class TestPyramidBlok:
    ...
    def test_no_client_id(self, registry_testblok, webserver):
        ...
        self.assert_missing_config(registry_testblok, "oidc_relying_party_client_id")
        
    def test_no_secret_id(self, registry_testblok, webserver):
        Configuration.set("oidc_provider_issuer", "http://fake")
        Configuration.set("oidc_relying_party_client_id", "abc")
        self.assert_missing_config(registry_testblok, "oidc_relying_party_secret_id")

    def test_no_rp_callback(self, registry_testblok, webserver):
        Configuration.set("oidc_provider_issuer", "http://fake")
        Configuration.set("oidc_relying_party_client_id", "abc")
        Configuration.set("oidc_relying_party_secret_id", "efg")
        self.assert_missing_config(registry_testblok, "oidc_relying_party_callback")
    ...
    def test_oidc_auth(self,mock_oidc, registry_testblok, webserver):
        ...
        Configuration.set("oidc_relying_party_callback", "http://localhost/oidc_callback")
        ...
        resp = webserver.get('/oidc_callback?code=a-fake-code&state={}'.format(qs["state"][0]), status=302)
